...
# sys 모듈 : 파이썬 인터프리터가 제공하는 여러가지 기능을 다룰수 있는 함수 제공
# exit : 스크립트 종료 함수
...
# 보통 언어,지역,코드 집합으로 구성됨 (kr_KR.UTF-8)
# 프로그램 내 UI의 텍스트가 올바르게 출력되도록 만듦


import locale
# 전역 로케일 설정 : LC_ALL (날짜,시간 등 모든 것을 설정)
...
# datetime 모듈
# time 모듈과 달리 시간대를 이용한 현지시간 출력,날짜/시간 차이 계산 기능이 추가
...
for _ in range(5):
    ...
    print(item,end=' ')
    menu.remove(item) #선택된 메뉴는 제거

# 비복워 추출 방식 로또 6/45 작성
lotto = []
for i in range(1,45+1):
    lotto.append(i)

for _ in range(6):
    lottos = random.choice(lotto)
    print(lottos, end = " ")
    lotto.remove(lottos)
...
person = {'name': '혜교', 'addr': '서울시 관악구', 'age': 32, 'email': 'abc123@987xyz.co.kr'}
