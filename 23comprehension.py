...
g = [val1[i]**2 for i in range(0,4+1)]
...
k = [ i for i in range(1,100+1) if i % 2 == 1]

# 1 ~ 100 사이의 정수 중 임의의 정수가 짝수면 even, 홀수면 odd 라고 구분해서 리스트에 저장하세요
l = []
...
# for 다중 if 축약문
# [표현식 if 조건 else 거짓일 때 표현식
#           for 항목 in 반복 가능 객체]

m = ['even' if i % 2 ==0 else 'odd' for i in range(1,100+1)]
...
# 다중 for 축약문
#[표현식 for 항목1 in 반복 가능 객체1
#       for 항목2 in 반복 가능 객체2]
o = [i*j for i in range(7,8+1)
         for j in range(1,9+1)]


# name, grd 리스트의 값들을 각각의 키와 값으로 딕셔너리를 만들어 저장하시오.
name = ['혜교','수지','지현']
grd = ['우','미','수']

# 리스트 2개 일 때 단순하고 무식한 버전
sj = {}
sj[name[0]] = grd[0]
sj[name[1]] = grd[1]
sj[name[2]] = grd[2]
...
paas = { key:val for key, val in zip(std,paskor)}

# 딕셔너리 for if 축약문 - {} 사용
# { 키 값 표현식 if 조건 else 거짓일 때 표현식
